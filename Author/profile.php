...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $gender = $_POST['gender'] ?? '';
    $short_bio = trim($_POST['short-bio']);
...
$total_posts = 0;
try {
    $posts = selectAll('post', ['users_id' => $user_id]);
...
<head>
    ...
    <link rel="stylesheet" href="header_author.css">
...
<body>
    ...
    <main class="main-content">
        <section>
            ...
            <form class="edit-profile-form" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <textarea id="short-bio" name="short-bio" class="form-input large-input"><?php 
                        echo is_array($user) && array_key_exists('bio', $user) 
        ...
        <section>
            ...
            <p class="bio">
                <p><strong>Gender: </strong> <?php echo htmlspecialchars($user['gender'] ?? ''); ?></p> <br>

                <?php if (is_array($user) && array_key_exists('bio', $user) && !empty($user['bio'])): ?>
                    <?php echo htmlspecialchars($user['bio']); ?>
                <?php else: ?>
                    No bio available<?php echo !is_array($user) || !array_key_exists('bio', $user) ? ' with current database structure' : ''; ?>.
                <?php endif; ?>
            </p> <br>
            <h4 class="post">Total posts published: <span><?php echo $total_posts; ?></span></h4>
            <p>You can contact me via <a href="mailto:<?php echo is_array($user) && isset($user['email']) ? htmlspecialchars($user['email']) : '#'; ?>">email</a>.</p>
        </section>
